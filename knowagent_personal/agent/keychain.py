...
def cmd_credential(params: dict) -> str:
    ...
    Actions:
      ...
      - **list**: list all credential names for the knowagent service
    ...
    Parameters:
        ...
        service (str, optional): Keychain service name. Default ``"knowagent"``.
    """
    action = params.get("action", "").strip().lower()
    account = params.get("name", "").strip()
    service = params.get("service", _KNOWAGENT_SERVICE).strip()
...
def migrate_plaintext_to_keychain(config: Config | None = None) -> str:
    ...
    for display_name, key_path_str, val in secrets_found:
        ...
        ok = keychain_set(_KNOWAGENT_SERVICE, account, val)
        if ok:
...
TOOL_SCHEMAS: dict = {
    "credential": {
        ...
        "properties": {
            ...
            "service": {
                "type": "string",
                "description": "Keychain service name, default 'knowagent'",
            },
        },
        "required": ["action"],
    },
