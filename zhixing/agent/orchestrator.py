...
class ToolResult:
    """一次工具调用的结果记录。"""
    def __init__(self, name: str, args: dict, output: str, success: bool, tool_call_id: str = ""):
        self.name = name
        self.args = args
        self.output = output
...
class Orchestrator:
    ...
    def execute_tool(self, name: str, args: dict, tool_call_id: str = "") -> ToolResult:
        handler = COMMANDS.get(name)
        if not handler:
            return ToolResult(name=name, args=args, output=f"命令 {name} 不存在", success=False, tool_call_id=tool_call_id)
        try:
            output = handler(args)
            return ToolResult(name=name, args=args, output=str(output), success=True, tool_call_id=tool_call_id)
        except Exception as e:
            return ToolResult(name=name, args=args, output=str(e), success=False, tool_call_id=tool_call_id)
    ...
    async def process(
        ...
        while round_count < max_tool_rounds:
            ...
            for tc in tool_calls:
                result = self.execute_tool(tc["name"], tc["arguments"], tool_call_id=tc.get("id", ""))
