...
class Agent:
    ...
    def __init__(self, llm_client, config):
        ...
        # 加载持久化权限规则
        import os as _os
        rules_path = _os.path.expanduser("~/.zhixing/permissions.json")
        if _os.path.exists(rules_path):
            self._harness.permissions.load_rules(rules_path)
        # 安装默认 Hooks
        from zhixing.harness.default_hooks import install_default_hooks
        install_default_hooks()
        # 从 SQLite 恢复持久化记忆（T2 用户偏好）
        db_path = config.get("storage.db_path", "~/.zhixing/personal.db")
        if db_path:
    ...
    def _ensure_rag(self):
        ...
        try:
            ...
            if self.rag.init():
                from zhixing.agent.tools import set_rag
                set_rag(self.rag)
                return True
        except Exception:
            self.rag = None
    ...
    def _load_history(self):
        ...
        try:
            from zhixing.memory.db import init_db, get_recent_messages
    ...
    def _save_message(self, role: str, content: str | None = None):
        ...
        try:
            from zhixing.memory.db import save_message
    ...
    def stop(self):
        ...
        if harness:
            # 持久化 T2 记忆到 SQLite
            db_path = self.config.get("storage.db_path", "~/.zhixing/personal.db")
            if db_path:
